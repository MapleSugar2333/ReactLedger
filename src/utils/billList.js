...
// 由年度账单筛出指定月的账单
const getMonthOverview = (data, month) => {
	// 某个月的账单可能有多个
	const bill = data.filter((item) => month === dayjs(item.date).get('month'));
	return getOverview(bill);
};

export {billListData, billTypeToName, getOverview, getMonthOverview};
